const DBService = require('./DBServiceSQL')

async function query(criteria = {}) {
  var namePart = criteria.name || ''
  var query = `SELECT * FROM bug  WHERE bug.title LIKE '%${namePart}%' OR bug.description LIKE '%${namePart}%'`
  const bugs = await DBService.runSQL(query)
  return bugs
}

async function getById(bugId) {
  var query = `SELECT * FROM bug WHERE bug._id = ${bugId}`

  var bugs = await DBService.runSQL(query)
  if (bugs.length === 1) return bugs[0]
  throw new Error(`bug id ${bugId} not found`)
}

async function add(bug) {
  var sqlCmd = `INSERT INTO bug (title, description, severity, creator_id) 
                VALUES ("${bug.title}",
                        "${bug.description}",
                        "${bug.severity}",
                        "${bug.creator_id}")`
  const newBug = await DBService.runSQL(sqlCmd)
  const currBug = await getById(newBug.insertId)
  return currBug
}

function save(bug) {
  if (bug._id) {
    return update(bug)
  } else {
    return add(bug)
  }
}

async function update(bug) {
  var query = `UPDATE bug set title = "${bug.title}",
                                description = "${bug.description}",
                                severity = ${bug.severity}
                WHERE bug._id = ${bug._id}`

  var okPacket = await DBService.runSQL(query)
  console.log(okPacket);
  if (okPacket.affectedRows !== 0) return okPacket
  throw new Error(`No bug updated - bug id ${bug._id}`)
}

function remove(bugId, userId) {
  // var query = `DELETE FROM bug WHERE bug._id = ${bugId}`
  var query = `DELETE FROM bug WHERE bug._id = ${bugId} && creator_id = ${userId}`

  return DBService.runSQL(query).then((okPacket) =>
    okPacket.affectedRows === 1
      ? okPacket
      : Promise.reject(new Error(`No bug deleted - bug id ${bugId}`)),
  )
}

module.exports = {
  query,
  getById,
  add,
  update,
  remove,
  save,
}
